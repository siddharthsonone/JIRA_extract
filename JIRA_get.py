...
for ticket in scope_tickets:
    ...
    while len(transitions) > 1:
        ...
        cursor_.execute( '''INSERT INTO JIRA_dump ([TICKET ID], [Summary], [Submitter], [PMS], [Quantity], [Zocdoc ProviderID], [Due Date], [DateTime] ,[Assignee], [Status From], [Status To],[Time Spent]) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')''' % (transitions[0][0], transitions[0][1], transitions[0][2], transitions[0][3], transitions[0][4], transitions[0][5], transitions[0][6], transitions[0][7], transitions[0][8], transitions[0][9], transitions[0][10], (transitions[1][7] - transitions[0][7])))
